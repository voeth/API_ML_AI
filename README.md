...
#### 3. 产品介绍
   1. 加值宣言：
      1. 主要功能都是调取百度AI开放平台的语音技术与自然语言处理技术的API来实现，使用[音频文件转写API](https://ai.baidu.com/ai-doc/ITMA/Qk38ikh93)实现音频转换为文本；
      2. 使用[评论观点抽取API](https://ai.baidu.com/tech/nlp_apply/comment_tag)提取文本中的关键信息，再使用[情感倾向分析](https://ai.baidu.com/tech/nlp_apply/sentiment_classify)判断对方的情感态度，可用于做数据统计。
   2. MVP核心加值：
...
2. [评论观点抽取API](https://ai.baidu.com/tech/nlp_apply/comment_tag)的工作原理主要是提取出一段话当中的实体词，省略虚词，不用担心重要信息的遗漏。
3. [情感倾向分析](https://ai.baidu.com/tech/nlp_apply/sentiment_classify)基于实体词的褒贬语义来判断情感倾向，但可能会忽略了具体语境。所以还得保留通话文本记录，交互界面上要将情感倾向的统计与评论观点的抽取放在同一个界面，方便察觉出概率性误差。
#### 4. 产品原型
   1. 原型2.信息设计
![快客app架构](image/快客摘取app.png )  

   2. 原型1交互及界面设计、原型3原型文档
   [快客app_prototype](https://voeth.github.io/API_ML_AI/快客app_prototype/)  
   
   3. API1.输入及输出数据、API4.加分项
   （使用了音频文件转写API、评论观点抽取API、情感倾向分析API，还调用pyecharts模块可视化api输出结果）
